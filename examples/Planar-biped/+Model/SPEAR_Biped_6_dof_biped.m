function  robot = SPEAR_Biped_6_dof_biped()
...
robot.Xtree{2} = eye(6);
robot.jtype{2}  = 'Pz';
robot.I{2} =  mcI(model_prms.M_torso, model_prms.Lcom_torso*robot.body_axis{2}, diag(1/12*model_prms.M_torso*model_prms.L_torso^2*[1 1 0])); % torso
%robot.appearance.body{2} = Anim.add_appearance('slender-rod', [], [], false);
body_appearance{1}.length = model_prms.L_torso;
body_appearance{1}.axis = robot.body_axis{2};
body_appearance{1}.color = model_prms.C_torso;
body_appearance{1}.radius = r_link;
robot.appearance.body{2} = Anim.add_appearance('slender-rod', body_appearance, [], false);
...
robot.Xtree{3} = eye(6);
robot.jtype{3}  = 'Ry';
robot.I{3} = mcI(model_prms.M_upper_leg, model_prms.Lcom_upper_leg*robot.body_axis{3}, diag(1/12*model_prms.M_upper_leg*model_prms.L_upper_leg^2*[-1 -1 0]));
...
robot.jtype{4}  = 'Ry';
robot.I{4} = mcI(model_prms.M_lower_leg, model_prms.Lcom_lower_leg*robot.body_axis{4}, diag(1/12*model_prms.M_lower_leg*model_prms.L_lower_leg^2*[-1 -1 0]));
...
robot.Xtree{5} = eye(6);
robot.jtype{5}  = 'Ry';
robot.I{5} = mcI(model_prms.M_upper_leg, model_prms.Lcom_upper_leg*robot.body_axis{5}, diag(1/12*model_prms.M_upper_leg*model_prms.L_upper_leg^2*[-1 -1 0]));
body_appearance{1}.length = model_prms.L_upper_leg;
...
robot.jtype{6}  = 'Ry';
robot.I{6} = mcI(model_prms.M_lower_leg, model_prms.Lcom_lower_leg*robot.body_axis{6}, diag(1/12*model_prms.M_lower_leg*model_prms.L_lower_leg^2*[-1 -1 0]));
body_appearance{1}.length = model_prms.L_lower_leg;
