...
NUD = numel(rbm.Inputs.uD.sym);
NUS = numel(rbm.Inputs.uS.sym);
...
%     if strcmp(nlp.VarsName{i},'input')
%         if length(nlp.VarsIdx{i})==numel(rbm.Inputs.uD.sym)
%             out.(join([nlp.VarsName{i},'_D'])) = [out.(nlp.VarsName{i}), w_opt(nlp.VarsIdx{i})];
%         end
%     else  
%         if i==283
%             qwe=0;
...
if isfield(out, 'aD')
   out.aD = reshape(out.aD, NUD, desired_trajectory.PolyOrder.Double+1); 
end

if isfield(out, 'aS')
   out.aS = reshape(out.aS, NUS, desired_trajectory.PolyOrder.Single+1); 
end
...
if desired_trajectory.Bool
    ...
    for i = 1:NCP
        if i<=round(NCP/2)
            out.y(:,i) = [full(funs.Y_Double_Controller(out.pos(:,i), out.aD, tau(i), t_plus, t_minus));0;0;];
            out.dy(:,i) = [full(funs.DY_Double_Controller(out.pos(:,i), out.vel(:,i), out.aD, tau(i), t_plus, t_minus));0;0;];
            out.ddy(:,i) = [full(funs.DDY_Double_Controller(out.pos(:,i), out.vel(:,i), out.acc(:,i), out.aD, tau(i), t_plus, t_minus));0;0;];
            out.zd_error(:,i) = out.ddy(:,i) + control_gain^2*out.y(:,i) + 2*control_gain*out.dy(:,i);
            
        else
            out.y(:,i) =full(funs.Y_Single_Controller(out.pos(:,i), out.aS, tau(i), t_plus, t_minus));
            out.dy(:,i) = full(funs.DY_Single_Controller(out.pos(:,i), out.vel(:,i), out.aS, tau(i), t_plus, t_minus));
            out.ddy(:,i) = full(funs.DDY_Single_Controller(out.pos(:,i), out.vel(:,i), out.acc(:,i), out.aS, tau(i), t_plus, t_minus));
            out.zd_error(:,i) = out.ddy(:,i) + control_gain^2*out.y(:,i) + 2*control_gain*out.dy(:,i);
